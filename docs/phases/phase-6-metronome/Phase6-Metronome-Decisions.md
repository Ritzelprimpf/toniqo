...
**Decision (revised 2026-05-21 after seeing UI mockups).** Icon + text. ▶ icon followed by "Start" in the stopped state; ⏸ icon followed by "Stop" in the running state. Both rendered together on the pill button, centered.

**Original decision (superseded).** Icon-only — rejected on review of the actual UI mockup because the 60dp-tall full-width pill is large enough that a lone icon felt visually sparse and weight-mismatched against the surrounding tempo card.

**Alternatives considered.**
- **Icon only.** Initially chosen, then revised. Mockup confirmed text+icon reads more confidently at this button size.
- **Text only.** Rejected — the icon adds quick visual recognition; removing it loses something for no gain.
- **Stop icon (⏹) instead of pause icon (⏸)** for semantic accuracy. Not adopted — ⏸ is conventional for "tap this to halt the audio," and the `DESIGN.md` §7 icon set already includes `play` and `pause`. The semantic note (metronome resets to beat 1 on next start; it does not resume from where it stopped) is documented in the plan, not encoded in the icon.

**Rationale (revised).**
- The button's visual weight (60dp tall, full width minus the tap-tempo circle) calls for content with substance.
- "Start" / "Stop" reads more confidently than a lone icon at that scale; the text confirms the action and improves accessibility (sighted users who don't read icons fluently still get a clear cue).
- Uses icons already present in the `DESIGN.md` §7 set; no new assets required.

**Implications for the Phase 6 plan.**
- Start/Stop pill button per `DESIGN.md` §8.2 dimensions (60dp tall, flex 1 width).
  - Stopped state: `play` icon + "Start" text, mint primary background with glow.
  - Running state: `pause` icon + "Stop" text, `bg.elev3` neutral background, no glow.
- Icon sits to the left of the text with consistent spacing (per `DESIGN.md` §8.2 — exact value to be verified there or raised as a conflict).
- Text strings are `R.string.metronome_start` / `R.string.metronome_stop` — already specified in `res/values/strings.xml` for accessibility content descriptions; now also rendered as visible labels.
- A note in `Phase6_4-PLAN.md`: the metronome has no pause-and-resume; stopping resets the next start to beat 1. The pause icon is used by convention, not because of pause semantics.
...
## Item 23 — UI elements emerging from mockup review (2026-05-21)

**Context.** UI mockups for the metronome (running 4/4 and stopped 7/8) revealed elements not explicitly covered by Items 1–22. None of these change the underlying behaviour; they refine the visible UI surface and bring documentation in line with the design.

**Decisions.**

### 23a — Page status kicker

A small status indicator at the top of the screen showing the metronome's running state:

- `METRONOME · RUNNING` with a leading mint pulsing dot, when `isPlaying = true`.
- `METRONOME · STOPPED` with no leading dot, when `isPlaying = false`.

Rendered in `mono.micro` (the kicker style). Sits above the page title "Metronome". This is consistent with other modules' status kickers (e.g., the tuner's listening/permission-denied status line).

### 23b — Beat indicator header line

A small mono-kicker label above the beat indicator row, split into two halves:

- **Left:** `BEAT · X / N` where `X` is the current main-beat number (1-indexed for display) and `N` is the time-signature numerator. Updates live as beats advance. When stopped, shows `BEAT · 1 / N` (i.e., previews where beat 1 is).
- **Right:** `QUARTER NOTES` for /4 signatures, `EIGHTH NOTES` for /8 signatures. Derived from `timeSignatureDenominator`. Makes the beat-unit semantics from Item 7 visible to the user.

Both halves use `mono.micro`. The right-side label sits right-aligned in the row.

### 23c — Subdivide kicker label

The kicker label above the subdivision dropdown reads **"SUBDIVIDE"** (verb form), not "SUBDIVISION" (noun). The label string lives at `R.string.metronome_subdivide_label`.

The dropdown *values* still use noun-form display strings ("None", "Eighth notes", "Sixteenth notes", "Eighth triplets") per Item 8's implications.

### 23d — Tempo card as a visually-grouped container

The BPM display, tempo descriptor, slider, and −/+ buttons live inside a single visually-grouped card (rounded border, slight elevation). The mockup makes this grouping explicit; Phase 6.4 should treat the tempo card as one composable container, not as five loose elements.

### 23e — Sun / theme-toggle icon at top-right

Present in the mockup at the top-right corner of the screen. This is **app-shell scaffolding**, not metronome-specific — it appears on every module screen and lives in the app-level top bar, not in `MetronomeScreen`. Phase 6.4 does not implement this; it's already part of the app shell from Phase 3 / Phase 4 (or, if it isn't yet, that's a separate concern from the metronome).

**Alternatives considered.**
- **Omit the status kicker.** Rejected — running/stopped state visible only via the Start/Stop pill is less prominent than feels right; the kicker echoes a pattern already established elsewhere in the app.
- **Omit the beat counter / beat-unit labels.** Rejected — the beat-unit label especially is the only place the BPM-refers-to-eighths-in-/8 semantics surfaces to the user; without it, Item 7 is invisible.
- **Use "SUBDIVISION" as the kicker.** Rejected — "SUBDIVIDE" (verb) reads more imperative-toolbar-y and matches the mockup's chosen language.

**Rationale.**
- The mockup is the design authority for visible structure. Mocked elements that aren't yet in the docs are gaps to close, not deviations to negotiate.
- Each added element serves a concrete user need: status kicker reinforces state, beat counter aids practice, beat-unit label demystifies time-signature semantics.

**Implications for the Phase 6 plan.**
- New child composables in `metronome/presentation/ui/`:
  - `MetronomeStatusKicker.kt` — renders the running/stopped indicator with the optional pulsing dot.
  - `BeatIndicatorHeader.kt` — renders the `BEAT · X / N` + beat-unit-name row above the beat segments.
- `MetronomeContent.kt` is updated to compose the status kicker at the top and the beat indicator header inline with the beat row.
- `BeatIndicator.kt` (or its surrounding container) takes the current beat number for the header display.
- New string resources:
  - `metronome_status_running` — "Metronome · Running"
  - `metronome_status_stopped` — "Metronome · Stopped"
  - `metronome_beat_header_format` — "Beat · %1$d / %2$d" (positional format args for current beat and numerator)
...
- A helper extension: `MetronomeConfig.beatUnitLabelResId: Int` returning the right string resource based on denominator.
- The pulsing dot in the status kicker uses the same mint signal color and a slow pulse animation (~1-second cycle) — implementation detail to be specified in Phase 6.4.
- The tempo card is rendered as a single `Card`-style composable wrapping its children. Exact corner radius, border, and elevation come from `DESIGN.md` §8.2 (or §6.6 if the project has a shared card primitive — Phase 4 introduced one for the Info section, which may be reusable here).

---

## All items resolved (revised)

The original 14-item sweep, the 8 follow-up items (15–22), the mockup-driven Item 23, and the revised Item 18 are all closed. Phase 6 is fully scoped at the requirements level. Phase 6.4's documents are being updated in lockstep to reflect Items 18 (revised) and 23.
